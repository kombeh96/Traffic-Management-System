...
#itms.geometry("300x175")
#directory = 'C:\Users\User\Downloads\Intelligent_Traffic_Management_System-main\Intelligent_Traffic_Management_System-main'
...
class SpecificationsFrame(tk.Toplevel):
	...
	def specificationsframe(self):
		...
		for i in range(len(self.originalFrame.specifications)):
			self.specificationsKey = tuple(self.originalFrame.specifications.keys())[i]
...
class RoadViewFrame(tk.Toplevel):
	...
	def roadViewFrame(self):
		...
		roadViewPanedWindow = tk.PanedWindow(self, height = 625, width = 900, orient = VERTICAL)
	...
	def animationFrame(self):
		...
		for k in range(2):
			...
			for i in range(len(self.laneInfoDict[k].keys())):
				text = tuple(self.laneInfoDict[k].keys())[i] + str(self.laneInfoDict[k][tuple(self.laneInfoDict[k].keys())[i]])
...
class CreateDatasetFrame(tk.Toplevel):
	...
	def createDatasetFrame(self):
		...
		self.update_idletasks()
		self.Frame = tk.Frame(self.mainFrame, height = 900, width = 2000)
		...
		for i in range(len(self.data.keys())):
			self.label[0][i] = tk.Label(self.frame[0], text = tuple(self.data.keys())[i], relief = RIDGE, width = columnWidth[i])#, height = 2)
...
class DisplayDatasetFrame(tk.Frame):
	...
	def __init__(self, parent=None):
		...
		self.main.title('Display Dataset')
		...
		df = pd.DataFrame(pd.read_csv('dataset/TrafficDataset' + str(100) + '.csv'))
...
class DataVisualizationFrame(tk.Toplevel):
	...
	def dataVisualiztaionFrame(self):
		vehicleNumberButton = tk.Button(self, text = 'Number of Vehicles in Each Lane', command = self.graph.createGraphs_vehicleNumber)
		...
		onTimeButton = tk.Button(self, text = 'Green Light On Time of Each Lane', command = self.graph.createGraphs_onTime)
...
class ITM(object):
	...
	def openframe(self, parent):
		...
		self.inputFolder.set(r'C:/Users/User/Downloads/Intelligent_Traffic_Management_System-main/Intelligent_Traffic_Management_System-main/input')
		...
		self.outputFolder.set(r'C:/Users/User/Downloads/Intelligent_Traffic_Management_System-main/Intelligent_Traffic_Management_System-main/output')
		...
		for i in range(len(self.specifications.keys())):
			self.specificationsKey = tuple(self.specifications.keys())[i]
